...
class Lunge:
    def __init__(self, reps, sets, rest_seconds=120):
        ...
        #what stage of the Lunge you are at
        self.stage = "Upright"
        # flag for resting 
    ...
    def get_WorkoutInfo(self, timestamp_ms):
        ...
        return [f"Lunges: {self.rep_count}/{self.reps}", f"Sets: {self.set_count}/{self.sets}", angle_text ]


    # this is called once a frame (by find_body()) and hold sthe main logic for a Lunge
    ...
    def process(self, landmarks, timestamp_ms, key=None):
        ...
        # get all the landmarks needed for a Lunge
        ...
        # the user has started descending for the lunge so state is Down 
        if angle < 145 and self.stage == "Upright":
            self.stage = "Down"
        ...
        # then the user is marked as standing upright completing one rep
        elif angle > 165 and self.stage == "Down":
            self.stage = "Upright"
            self.rep_count += 1
            ...
            if self.rep_count >= self.reps:
                ...
                if self.set_count >= self.sets:
                    self.complete = True
                    message = "Lunges Complete! Great Job!"
