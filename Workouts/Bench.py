...
class Bench:
    def __init__(self, reps, sets, rest_seconds=120):
        ...
        #what stage of the bench you are at
        self.stage = "Upright"
        # flag for resting 
    ...
    def get_WorkoutInfo(self, timestamp_ms):
        ...
        return [f"Bench: {self.rep_count}/{self.reps}", f"Sets: {self.set_count}/{self.sets}", angle_text ]
    ...
    def process(self, landmarks, timestamp_ms, key=None):
        ...
        # the user has started descending for the bench so state is Down 
        if angle < 100 and self.stage == "Upright":
            ...
            if self.rep_count >= self.reps:
                ...
                if self.set_count >= self.sets:
                    self.complete = True
                    message = "Bench Complete! Great Job!"
